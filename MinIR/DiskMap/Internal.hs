{-# LANGUAGE DeriveGeneric, TemplateHaskell, FlexibleContexts, FlexibleInstances, TypeSynonymInstances #-}

module MinIR.DiskMap.Internal ( DiskMap
                              , MemTree, DiskTree
                              , empty
                              , insert, insertWith
                              , lookup
...
instance Binary (ObjUpdate a) where
    ...
    put (NewObj _)     = error "DiskMap: Tried to serialize NewObj"
...
instance (Binary v) => Binary (DiskTree v) where
    ...
    get = do type_ <- getWord8
             case type_ of
                 ...
                 i -> fail $ "DiskMap: Unknown tree type ("++show i++")"

data DiskMap v = DMap { _store :: DS.DiskStore (DiskTree v) }
makeLenses ''DiskMap

lookup :: (Binary v, Show v) => Key -> DiskMap v -> IO (Maybe v)
lookup k dmap = getRoot dmap >>= lookup' dmap k

lookup' :: (Binary v, Show v) => DiskMap v -> Key -> MemTree v -> IO (Maybe v)
...
insert :: (Binary v, Show v) => DiskMap v -> Key -> v -> MemTree v -> IO (MemTree v)
...
insertWith :: (Binary v, Show v)
           => DiskMap v -> (v -> v -> v) -> Key -> v -> MemTree v -> IO (MemTree v)
...
insertWith' :: (Binary v, Show v)
            => DiskMap v -> (v -> v -> v) -> Key -> v -> MemTree v -> IO (Insert v)
...
insertWith' dmap f k v (Internal n children) = do
    ...
    case maybeChild of
      Nothing -> error "DiskMap: Internal node not found"
      Just child -> do
  ...
  where splitInternal :: Tree v -> Insert v
        splitInternal (Leaf _ _) = error "DiskMap: splitInternal can't split Leafs"
        splitInternal (Internal n (Bin children c))
          | n > maxBreadth =
            let (l,r) = divide children
                ...
                --nr = n - nl -- FIXME
                nr = IM.size r + 1
            in --trace ("\n   split internal n="++show n++
               --  "  origKeys="++show (IM.keys children)++
               --  "  keys="++show(IM.keys l,IM.keys r)++
               --  "  splitKey="++show kr'++"\n") $
               Split kr' (Internal nl (Bin l r')) (Internal nr (Bin r c))
          | otherwise      = Ok $ Internal n (Bin children c)

writeChildren :: (Binary v) => DiskMap v -> MemTree v -> IO (DiskTree v)
writeChildren _                 (Leaf n values)       = return (Leaf n values)
writeChildren dmap@(DMap store) (Internal n children) =
...
writeTree :: (Binary v) => DiskMap v -> MemTree v -> IO (DiskTree v, DS.Obj (DiskTree v))
writeTree dmap@(DMap store) tree = do
...
readChild :: (Binary v)
          => DiskMap v -> Key -> MemTree v -> IO (Maybe (MemTree v))
readChild (DMap store) _ (Leaf _ _)          = return Nothing
readChild (DMap store) k (Internal _ children) =
...
putRoot :: (Binary v) => DiskMap v -> MemTree v -> IO (DiskTree v)
putRoot dmap@(DMap store) tree = do
    (tree', obj) <- writeTree dmap tree
    void $ DS.setRoot store (Just obj)
    return tree'

getRoot :: (Binary v) => DiskMap v -> IO (MemTree v)
getRoot (DMap store) = maybe (error "DiskMap: Empty root") id <$> DS.getRoot store
...
new :: Binary v => FilePath -> IO (DiskMap v)
new fname = do
    store <- DS.new fname
...
open :: Binary v => FilePath -> IO (Maybe (DiskMap v))
open fname = fmap DMap <$> DS.open fname
